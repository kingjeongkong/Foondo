...
import type { RecommendationResponse } from '@/app/types/recommendations';
import type { ScoredRestaurant } from '@/app/types/restaurant';
...
/**
 * `calculateRestaurantScores` 결과(랭킹된 ScoredRestaurant 배열)를
 * SSE `result` 이벤트로 보낼 RecommendationResponse 형태로 매핑합니다.
 *
 * - finalScore는 소수 첫째 자리 반올림
 * - 응답에 노출할 필드만 골라 담음 (id/placeId/name/address/photoReference,
 *   각 score, aiSummary)
 */
export function buildRecommendationPayload(
  restaurantScores: ScoredRestaurant[],
  message: string
