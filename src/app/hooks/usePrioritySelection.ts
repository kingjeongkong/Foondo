...
export function usePrioritySelection(initialItems: PriorityItem[]) {
  ...
  // 항목 선택 (탭으로 선택)
  const selectPriority = useCallback((item: PriorityItem) => {
    setState((prev) => {
      // 3개 선택 완료 시 제한
      if (prev.selected.length >= 3) return prev;
  ...
  // 항목 해제 (탭으로 해제)
  const deselectPriority = useCallback((rank: 1 | 2 | 3) => {
    setState((prev) => {
      const targetItem = prev.selected.find((s) => s.rank === rank);
  ...
  const getPrioritySettings = useCallback((): PrioritySettings => {
    ...
    state.selected.forEach(({ rank, item }) => {
      if (item.id in settings) {
        settings[item.id as keyof PrioritySettings] = rank;
      }
    });
